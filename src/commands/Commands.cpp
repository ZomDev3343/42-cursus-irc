...
void Commands::handleDCC(IrcServer &server, IrcClient &user, const std::string &target, const std::string &ctcpMessage)
{
    IrcClient *targetClient = server.getClient(target);
    if (!targetClient)
    {
...
void Commands::join_command(IrcServer &server, IrcClient &user, std::string command)
{
    ...
    if (!channel)
    {
        ...
        // Add the bot to the channel without making it an operator
        Bot* bot = server.getBot();
        if (bot)
        {
            channel->addClient(bot);
            // Broadcast that the bot has joined the channel
